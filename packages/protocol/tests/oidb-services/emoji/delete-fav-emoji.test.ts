...
import { DeleteFavEmoji } from '../../../src/oidb-services/emoji/delete-fav-emoji';
...
describe('DeleteFavEmoji namespace', () => {
  describe('serialize + encode', () => {
    it('reproduces the captured delete wire bytes exactly', () => {
      const req = DeleteFavEmoji.serialize({} as any, { uin: SAMPLE_UIN, emojiId: SAMPLE_EMOJI_ID });
      const hex = Buffer.from(DeleteFavEmoji.encode(req)).toString('hex');
      expect(hex).toBe(DELETE_WIRE_HEX);
    });

    it('omits qqVersion from inner (delete inner is 14 bytes, not 28)', () => {
      const req = DeleteFavEmoji.serialize({} as any, { uin: SAMPLE_UIN, emojiId: 'x' });
  ...
  describe('invoke', () => {
    it('routes through "Faceroam.OpReq"', async () => {
      ...
      await DeleteFavEmoji.invoke(sender, { uin: SAMPLE_UIN, emojiId: SAMPLE_EMOJI_ID });
    ...
    it('resolves on success without returning a body', async () => {
      const sender = makeSender();
      await expect(DeleteFavEmoji.invoke(sender, { uin: SAMPLE_UIN, emojiId: 'x' })).resolves.toBeUndefined();
    });
    ...
    it('throws when the sender reports no response', async () => {
      ...
      await expect(DeleteFavEmoji.invoke(sender, { uin: SAMPLE_UIN, emojiId: 'x' })).rejects.toThrow();
    });
  });
});
