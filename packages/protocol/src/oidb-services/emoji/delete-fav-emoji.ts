// Faceroam.OpReq opType=2 — 删除一个收藏表情。
//
// 和 fetch 同一个 trpc service，区别在 field3=2、inner 不带 qqVersion、
// 业务体 field5 塞 emoji_id。emoji_id 格式是 `{uin 段}_0_0_0_{MD5}_0_0`，
// 直接用 fetch 响应里解析出来的值回传即可，不用自己拼。field2 同样是
// 当前账号 UIN。
//
...
export namespace DeleteFavEmoji {
